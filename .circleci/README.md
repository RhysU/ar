...
This directory contains the CircleCI configuration for automated testing and documentation deployment.

## Jobs

### `test`
Runs the test suite across multiple Python versions (3.7-3.13) with different NumPy versions.

### `deploy-docs`
Builds Doxygen documentation and deploys it to the `gh-pages` branch on GitHub Pages.
This job only runs on the `master` branch after all tests pass successfully.

## GitHub Pages Deployment Setup

To enable automatic documentation deployment, you need to configure the following in your CircleCI project settings:
...
3. **GH_TOKEN**: GitHub Personal Access Token with `repo` scope
   ...
   - This allows CircleCI to push to the `gh-pages` branch

### GitHub Repository Setup

1. The `gh-pages` branch will be automatically created on the first deployment
