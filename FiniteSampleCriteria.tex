...
Emphasis is placed on converting the criteria formulas into forms efficiently
computable when evaluating a single result.  Cases where many sequential
evaluations are required may employ intermediate results.
...
\begin{align}
    \text{overfit}\!\left(\text{CIC},N,p,v_\ast\right)
    &=
    \max\left(
        \text{overfit}\!\left(\text{FSIC},N,p,v_\ast\right),
        \text{overfit}\!\left(\text{FIC},N,p,v_\ast,3\right)
    \right)
    .
\end{align}
