...
def main():
    ...
    result = ar.arsel(np.array([data]), False, True, 'CIC', len(expected), len(expected))
    estimated = result.AR[0][1:]

    if len(estimated) != len(expected):
        raise ValueError("Expected {} coefficients, got {}".format(len(expected), len(estimated)))

    # Compare coefficients
    print("{:>22s} {:>22s} {:>22s}".format("Nominal Value", "arsel Result", "Percent Diff"))
    print("{:>22s} {:>22s} {:>22s}".format("-------------", "------------", "------------"))

    max_error = 0.0
    for exp, est in zip(expected, estimated):
        diff = (est - exp) / exp * 100.0 if exp != 0 else 0.0 if est == 0.0 else float('inf')
    ...
    if max_error > 0.01:
        raise ValueError("Maximum error {:.6g}% exceeds tolerance 0.01%".format(max_error))
